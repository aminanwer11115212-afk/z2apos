...
function Dashboard() {
  ...
  const stats = useQuery({
    ...
    queryFn: async () => {
      ...
      const [
        ...
        supabase
          .from("sales")
          .select("total,discount,tax_amount,paid,created_at")
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        days.push({ day: AR_DAYS[d.getDay()], total: Math.round(total) });
      }
  ...
  const cards = [
    {
      label: "مبيعات اليوم",
      value: formatSDG(stats.data?.todaySales ?? 0),
      Icon: ShoppingCart,
      color: "bg-primary/10 text-primary",
    },
    {
      label: "فواتير اليوم",
      value: String(stats.data?.salesCount ?? 0),
      Icon: TrendingUp,
      color: "bg-success/10 text-success",
    },
    {
      label: "أصناف المخزون",
      value: String(stats.data?.partsCount ?? 0),
      Icon: Package,
      color: "bg-accent text-accent-foreground",
    },
    {
      label: "أصناف منخفضة",
      value: String(stats.data?.lowStock ?? 0),
      Icon: AlertTriangle,
      ...
      <div className="grid lg:grid-cols-3 gap-3">
          ...
          <div className="h-56" dir="ltr">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={stats.data?.days ?? []}>
                ...
                />
                <Tooltip
                  formatter={(v: number) => [formatSDG(v), "المبيعات"]}
                  contentStyle={{
                    background: "var(--card)",
        ...
        <div className="bg-card border rounded-2xl p-4">
          <div className="flex items-center justify-between mb-3">
            <h2 className="font-bold">أصناف منخفضة</h2>
            <Link to="/parts" className="text-xs text-primary hover:underline">
              عرض الكل
            </Link>
          </div>
          {!stats.data?.lowStockItems?.length ? (
            <p className="text-sm text-muted-foreground py-4 text-center">
              لا توجد أصناف منخفضة 🎉
            </p>
          ) : (
            <ul className="space-y-2">
              {stats.data.lowStockItems.map((p) => (
                <li
                  key={p.id}
                ...
                >
                  ...
                  <span className="text-destructive font-semibold text-sm shrink-0 pr-2">
                    {Number(p.quantity)}
                  </span>
                </li>
              ))}
...
function QuickLink({
  ...
  return (
    <Link
      to={to}
      className={`rounded-2xl p-4 border flex items-center gap-3 font-semibold ${primary ? "bg-primary text-primary-foreground border-primary" : "bg-card hover:bg-muted"}`}
    >
