...
import { Package, ShoppingCart, AlertTriangle, Users, TrendingUp } from "lucide-react";
    ...
    queryFn: async () => {
      ...
      for (let i = 6; i >= 0; i--) {
        ...
        const total = (weekSales ?? [])
          ...
          .reduce((s, r) => s + Number(r.total) - Number(r.discount), 0);
      ...
      <div className="grid lg:grid-cols-3 gap-3">
        ...
        <div className="bg-card border rounded-2xl p-4">
          <div className="flex items-center justify-between mb-3">
            ...
            <Link to="/parts" className="text-xs text-primary hover:underline">عرض الكل</Link>
          </div>
          {!stats.data?.lowStockItems?.length ? (
            <p className="text-sm text-muted-foreground py-4 text-center">لا توجد أصناف منخفضة 🎉</p>
          ) : (
            <ul className="space-y-2">
              {stats.data.lowStockItems.map((p) => (
                <li key={p.id} className="flex justify-between items-center text-sm py-1.5 border-b last:border-0">
                  <div className="min-w-0">
                    <div className="font-medium truncate">{p.name}</div>
                    <div className="text-xs text-muted-foreground font-mono">{p.code}</div>
                  </div>
                  <span className="text-destructive font-semibold text-sm shrink-0 pr-2">{Number(p.quantity)}</span>
                </li>
              ))}
            </ul>
          )}
