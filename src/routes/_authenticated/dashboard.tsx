...
function Dashboard() {
  const stats = useQuery({
    ...
    queryFn: async () => {
      ...
      const [{ data: weekSales }, { data: parts }, { count: partsCount }, { count: custCount }, { data: lowStockList }, { data: custBalances }, { data: suppBalances }] = await Promise.all([
      ...
      const custDebts = (custBalances ?? []).reduce((s, r) => s + Number(r.balance), 0);
      const suppDues = (suppBalances ?? []).reduce((s, r) => s + Number(r.balance), 0);
      return { todaySales, salesCount: todaySalesArr.length, partsCount: partsCount ?? 0, custCount: custCount ?? 0, lowStock, days, lowStockItems, custDebts, suppDues };
    },
  });
  ...
  const cards = [
    ...
    { label: "ديون العملاء", value: formatSDG(stats.data?.custDebts ?? 0), Icon: Users, color: "bg-destructive/10 text-destructive", to: "/customers" },
    { label: "مستحقات الموردين", value: formatSDG(stats.data?.suppDues ?? 0), Icon: Wallet, color: "bg-warning/20 text-warning-foreground", to: "/suppliers" },
  ...
  ] as { label: string; value: string; Icon: typeof Package; color: string; to?: string }[];
        ...
        {cards.map((c) => {
          const body = (
            <>
              ...
              <div className="mt-3 text-xl font-bold truncate">{c.value}</div>
              <div className="text-xs text-muted-foreground mt-1">{c.label}</div>
            </>
          );
          return c.to
            ? <Link key={c.label} to={c.to} className="bg-card border rounded-2xl p-4 hover:bg-muted transition-colors block">{body}</Link>
            : <div key={c.label} className="bg-card border rounded-2xl p-4">{body}</div>;
        })}
      </div>
