...
function PurchaseView() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("purchases")
        .select(
          "id,invoice_no,total,paid,created_at,notes,payment_method,account_name,supplier_id, suppliers(id,name,phone,balance), purchase_items(id,qty,unit_cost,subtotal, parts(name,code))",
        )
        .eq("id", id)
        .single();
  ...
  useEffect(() => {
    document.title = data ? `شراء #${data.invoice_no} — 2A` : "فاتورة شراء — 2A";
  }, [data]);
          ...
          >
            <div className="flex items-start justify-between gap-4 pb-3 border-b">
              <div className="flex items-center gap-3">
                {settings.showLogo && (
                  <Logo variant="light" className={isThermal ? "h-10 w-auto" : "h-14 w-auto"} />
                )}
                <div>
                  ...
                  {settings.storeAddress && (
                    <div className="text-xs muted-print text-muted-foreground hide-on-thermal">
                      {settings.storeAddress}
                    </div>
                  )}
                </div>
              ...
              <div className="text-left">
                ...
                <div className={`font-bold font-mono ${isThermal ? "text-lg" : "text-2xl"}`}>
                  #{data.invoice_no}
                </div>
                ...
                {data.purchase_items.map((it) => (
                  <tr key={it.id}>
                    <td className="py-2">
                      ...
                      {it.parts?.code && (
                        <div className="text-xs muted-print text-muted-foreground font-mono">
                          {it.parts.code}
                        </div>
                      )}
                    </td>
      ...
      {data.suppliers && (
        <PaymentDialog
          ...
          party={{
            ...
            balance: Number(data.suppliers.balance),
          }}
          purchaseId={data.id}
          suggested={due}
        />
