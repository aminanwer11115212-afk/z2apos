...
function InvoicePaymentPage() {
  ...
  const { data: sale, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("sales")
        .select(
          "id,invoice_no,total,discount,tax_amount,paid,created_at,notes,payment_method,account_name,customer_id, customers(id,name,phone,balance)",
        )
        .eq("id", id)
        .single();
  ...
  return (
    <div className="p-4 lg:p-6 max-w-2xl mx-auto">
      ...
      <PageHeader
        title={`تسجيل دفعة — فاتورة #${sale.invoice_no}`}
        subtitle={sale.customers?.name ?? "نقدي"}
      />
      <div className="bg-card border rounded-2xl p-4 space-y-4">
        <div className="grid grid-cols-3 gap-2 text-center">
          <div className="p-3 rounded-lg bg-muted/50">
            <div className="text-xs text-muted-foreground">الإجمالي</div>
            <div className="font-bold">{formatSDG(grand)}</div>
          </div>
          <div className="p-3 rounded-lg bg-muted/50">
            <div className="text-xs text-muted-foreground">المدفوع</div>
            <div className="font-bold">{formatSDG(Number(sale.paid))}</div>
          </div>
          <div className="p-3 rounded-lg bg-muted/50">
            <div className="text-xs text-muted-foreground">المتبقي</div>
            <div className="font-bold text-destructive">{formatSDG(due)}</div>
          </div>
        </div>
        <Field label="المبلغ *">
          <Input
        ...
        <button onClick={() => setAmount(due)} className="text-xs text-primary hover:underline">
          تعيين المبلغ المتبقي
        </button>
        <Field label="طريقة الدفع">
          <div
            className="grid gap-2"
            style={{
              gridTemplateColumns: `repeat(${Math.max(1, methods.length)}, minmax(0, 1fr))`,
            }}
          >
            {methods.map((m) => (
              <button
        ...
        <div className="flex gap-2 pt-2">
          <Link to="/sales/$id" params={{ id }} className="flex-1">
