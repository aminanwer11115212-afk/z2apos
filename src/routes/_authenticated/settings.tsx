    ...
    <div className="p-4 lg:p-6 max-w-4xl mx-auto space-y-6">
      ...
      <Section title="الشعار المخصّص" icon={<ImageIcon className="w-4 h-4" />}>
        <div className="flex items-center gap-4">
          <div className="w-24 h-24 rounded-xl border bg-muted flex items-center justify-center overflow-hidden">{s.customLogoUrl ? <img src={s.customLogoUrl} alt="logo" className="max-w-full max-h-full" /> : <span className="text-xs text-muted-foreground">الافتراضي</span>}</div>
          <div className="flex-1 space-y-2">
            ...
            <p className="text-xs text-muted-foreground">PNG/JPG صغيرة — يفضّل بخلفية شفافة.</p>
          </div>
        </div>
      </Section>
      ...
      <Section title="إعدادات الطباعة" icon={<Printer className="w-4 h-4" />}>
        <Field label="نوع الطابعة"><div className="grid grid-cols-3 gap-2">{([{ v: "a4", label: "A4 عادي" }, { v: "thermal80", label: "حراري 80mm" }, { v: "thermal58", label: "حراري 58mm" }] as { v: PrintFormat; label: string }[]).map((o) => <button key={o.v} onClick={() => saveSettings({ printFormat: o.v })} className={`h-11 rounded-lg border text-sm font-medium ${s.printFormat === o.v ? "bg-primary text-primary-foreground border-primary" : "hover:bg-muted"}`}>{o.label}</button>)}</div></Field>
