...
const PAY_META: Partial<Record<PaymentMethodKey, { icon: string; label: string }>> = {
...
function SettingsPage() {
    ...
    saveSettings({
      accounts: [
        ...s.accounts,
        {
          ...
          type: newAcc.type,
          note: newAcc.note.trim() || undefined,
        },
      ],
    });
  ...
  const remove = (id: string) => {
    if (s.accounts.length <= 1) return toast.error("يجب حساب واحد على الأقل");
    const next = s.accounts.filter((a) => a.id !== id);
    saveSettings({
      accounts: next,
      defaultAccountId: s.defaultAccountId === id ? next[0].id : s.defaultAccountId,
    });
  };
  const accIcon = (type: AccountType) =>
    ACC_TYPES.find((t) => t.v === type)?.icon ?? <CreditCard className="w-5 h-5" />;
  const accLabel = (type: AccountType) => ACC_TYPES.find((t) => t.v === type)?.label ?? type;
  const updPay = (k: PaymentMethodKey, p: object) =>
    saveSettings({
      paymentMethods: { ...s.paymentMethods, [k]: { ...s.paymentMethods[k], ...p } },
    });
  const eligible = (k: PaymentMethodKey) =>
    k === "cash"
      ? s.accounts.filter((a) => a.type === "cash")
      : s.accounts.filter((a) => a.type === "bank" || a.type === "wallet");
    ...
    <div className="p-4 lg:p-6 max-w-4xl mx-auto space-y-6">
      ...
      <Section title="بيانات المتجر" icon={<Wallet className="w-4 h-4" />}>
        <div className="grid sm:grid-cols-2 gap-3">
          ...
          <Field label="الهاتف">
            <Input
              value={s.storePhone}
              onChange={(e) => saveSettings({ storePhone: e.target.value })}
            />
          </Field>
          <Field label="العنوان">
            <Input
          ...
          <Field label="الرقم الضريبي / السجل">
            <Input
              value={s.storeTaxNo}
              onChange={(e) => saveSettings({ storeTaxNo: e.target.value })}
            />
          </Field>
          <Field label="نص أسفل الفاتورة">
            <Input
              value={s.invoiceFooter}
              onChange={(e) => saveSettings({ invoiceFooter: e.target.value })}
            />
          </Field>
        </div>
      </Section>
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          ...
          <Field label="بادئة رقم الفاتورة">
            <Input
              ...
              onChange={(e) => saveSettings({ invoicePrefix: e.target.value })}
              placeholder="INV-"
            />
          </Field>
          <Field label="عدد نسخ الطباعة">
            <Input
              type="number"
              min={1}
              max={5}
              value={s.printCopies}
              onChange={(e) =>
                saveSettings({ printCopies: Math.max(1, Number(e.target.value) || 1) })
              }
            />
          </Field>
          <Field label="حد المخزون المنخفض">
            <Input
              type="number"
              min={0}
              value={s.lowStockDefault}
              onChange={(e) =>
                saveSettings({ lowStockDefault: Math.max(0, Number(e.target.value) || 0) })
              }
            />
          </Field>
        </div>
            ...
            <div key={a.id} className="flex items-center gap-3 p-3 rounded-lg border bg-card">
              ...
              <label className="flex items-center gap-1 text-xs">
                <input
                  type="radio"
                  name="def-acc"
                  checked={s.defaultAccountId === a.id}
                  onChange={() => saveSettings({ defaultAccountId: a.id })}
                />
                افتراضي
              </label>
              <button
      ...
      <Section title="طرق الدفع" icon={<Wallet className="w-4 h-4" />}>
        <p className="text-xs text-muted-foreground mb-3">
          تفعيل/تعطيل الطرق، اختيار الحساب الافتراضي، وإلزام رقم العملية.
        </p>
            ...
            <div key={k} className="p-3 rounded-lg border bg-card mb-2">
              <div className="flex items-center justify-between gap-2 flex-wrap mb-3">
                <div className="font-medium">
                  {PAY_META[k]?.icon} {PAY_META[k]?.label}
                </div>
                <div className="flex items-center gap-3 text-xs">
                  <label className="flex items-center gap-1">
                    <input
                      type="checkbox"
                      checked={cfg.enabled}
                      onChange={(e) => updPay(k, { enabled: e.target.checked })}
                    />
                    مفعّلة
                  </label>
                  <label className="flex items-center gap-1">
              ...
              <div className="grid sm:grid-cols-2 gap-2">
                <Field label="الحساب الافتراضي">
                  <select
                    ...
                    onChange={(e) => updPay(k, { defaultAccountId: e.target.value })}
                    className="w-full h-10 px-2 rounded-lg border bg-background text-sm"
                  >
                    {el.length === 0 && <option value="">— لا يوجد حساب —</option>}
                    {el.map((a) => (
                ...
                <Field label="رقم العملية">
                  <label className="flex items-center gap-2 h-10 px-3 rounded-lg border bg-background cursor-pointer text-sm">
                    <input
                      type="checkbox"
                      checked={cfg.requireRef}
                      onChange={(e) => updPay(k, { requireRef: e.target.checked })}
                    />
                    يتطلب رقم عملية
                  </label>
                </Field>
        ...
        <div className="grid sm:grid-cols-3 gap-3">
          <Field label="رؤية سعر التكلفة">
            <label className="flex items-center gap-2 h-11 px-3 rounded-lg border bg-background cursor-pointer">
              <input
                type="checkbox"
                checked={s.sellerPerms.seeCost}
                onChange={(e) =>
                  saveSettings({ sellerPerms: { ...s.sellerPerms, seeCost: e.target.checked } })
                }
              />
          ...
          <Field label="تعديل السعر في البيع">
            <label className="flex items-center gap-2 h-11 px-3 rounded-lg border bg-background cursor-pointer">
              <input
                type="checkbox"
                checked={s.sellerPerms.editPrice}
                onChange={(e) =>
                  saveSettings({ sellerPerms: { ...s.sellerPerms, editPrice: e.target.checked } })
                }
              />
          ...
          <Field label="أقصى نسبة خصم %">
            <Input
              ...
              min={0}
              max={100}
              value={s.sellerPerms.maxDiscountPercent}
              onChange={(e) =>
                saveSettings({
                  sellerPerms: {
                    ...s.sellerPerms,
                    maxDiscountPercent: Math.max(0, Math.min(100, Number(e.target.value) || 0)),
                  },
                })
              }
            />
