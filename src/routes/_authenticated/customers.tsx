...
function CustomersPage() {
  ...
  return (
    <div className="p-4 lg:p-6 max-w-5xl mx-auto">
      ...
      />
      <div className="mb-4">
        <SearchBar value={q} onChange={setQ} placeholder="بحث بالاسم أو الهاتف..." />
      </div>
            ...
            <tbody>
              {filtered.map((c) => (
                <tr key={c.id} className="border-t hover:bg-muted/50">
                  ...
                  <td
                    className={`p-3 font-semibold ${Number(c.balance) > 0 ? "text-destructive" : ""}`}
                  >
                    {formatSDG(c.balance)}
                  </td>
                  <td className="p-3 whitespace-nowrap">
                    ...
                    <button onClick={() => openEdit(c)} className="p-2 hover:bg-muted rounded-lg">
                      <Pencil className="w-4 h-4" />
                    </button>
                    <button
                      onClick={() => {
                        if (confirm("حذف هذا العميل؟")) del.mutate(c.id);
                      }}
