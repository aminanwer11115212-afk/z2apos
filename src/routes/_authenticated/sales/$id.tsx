...
import { useSettings, parseNotes, saleTotals, formatInvoiceNo, renderTemplate } from "@/lib/settings";
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("sales")
        .select("id,invoice_no,total,discount,tax_amount,paid,created_at,notes,payment_method,account_name,customer_id, customers(id,name,phone,balance), sale_items(id,qty,unit_price,subtotal, parts(name,code))")
        .eq("id", id).single();
