...
function SupplierStatement() {
  ...
  const { data: rows = [] } = useQuery({
    ...
    queryFn: async () => {
      const [pRes, payRes] = await Promise.all([
        supabase
          .from("purchases")
          .select("id,invoice_no,total,paid,created_at,notes")
          .eq("supplier_id", id)
          .order("created_at"),
        supabase
          .from("payments")
          .select("id,amount,method,account_name,notes,created_at,purchase_id")
          .eq("supplier_id", id)
          .eq("direction", "out")
          .order("created_at"),
      ]);
      ...
      for (const pay of payRes.data ?? []) {
        if (pay.purchase_id)
          linkedToPurchase.set(
            pay.purchase_id,
            (linkedToPurchase.get(pay.purchase_id) ?? 0) + Number(pay.amount),
          );
      }
      ...
      for (const pay of payRes.data ?? [])
        list.push({
          ...
          id: pay.id,
          date: pay.created_at,
          ref: `سداد${pay.purchase_id ? " (على فاتورة)" : ""}`,
          debit: Number(pay.amount),
          credit: 0,
          note: `${paymentMethodLabel(pay.method)}${pay.account_name ? " · " + pay.account_name : ""}${pay.notes ? " · " + pay.notes : ""}`,
        });
  ...
  const waHref = sup.phone
    ? whatsappUrl(
        sup.phone,
        renderTemplate(settings.waReminderTemplate, {
          name: sup.name,
          balance: formatSDG(sup.balance),
      ...
      <div className="print-area bg-card border rounded-2xl p-5 shadow-sm">
        <div className="flex items-start justify-between gap-4 pb-3 border-b">
          ...
          <div className="text-left text-xs muted-print text-muted-foreground">
            <div>
              المورد: <span className="font-bold text-foreground">{sup.name}</span>
            </div>
            {sup.phone && <div>هاتف: {sup.phone}</div>}
            <div>
              طُبع: {new Date().toLocaleString("ar-SD", { dateStyle: "short", timeStyle: "short" })}
            </div>
          </div>
        </div>
        ...
        <table className="w-full text-sm mt-3">
          ...
          <tbody className="divide-y">
            {rows.length === 0 ? (
              ...
              rows.map((r) => {
                ...
                return (
                  <tr key={`${r.kind}-${r.id}`}>
                    <td className="py-2 text-xs muted-print text-muted-foreground">
                      {new Date(r.date).toLocaleDateString("ar-SD")}
                    </td>
                    <td className="py-2">
                      <div className="font-medium">{r.ref}</div>
                      {r.note && (
                        <div className="text-xs muted-print text-muted-foreground">{r.note}</div>
                      )}
                    </td>
                    <td className="py-2 text-center">{r.debit ? formatSDG(r.debit) : "—"}</td>
                    <td className="py-2 text-center">{r.credit ? formatSDG(r.credit) : "—"}</td>
                    <td
                      className={`py-2 text-left font-semibold ${running > 0 ? "text-destructive" : "text-success"}`}
                    >
                      {formatSDG(running)}
                    </td>
                  </tr>
          ...
          <tfoot className="border-t font-bold">
            <tr>
              ...
              <td
                className={`py-2 text-left ${totals.balance > 0 ? "text-destructive" : "text-success"}`}
              >
                {formatSDG(totals.balance)}
              </td>
            </tr>
          </tfoot>
        </table>
