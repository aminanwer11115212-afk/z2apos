...
function NewPurchase({ onDone, onCancel }: { onDone: () => void; onCancel: () => void }) {
  ...
  const save = useMutation({
    ...
    onSuccess: (p) => {
      toast.success(`تم حفظ فاتورة الشراء #${p.invoice_no}`);
      onDone();
    },
                  ...
                  <div key={l.part.id} className="p-3 flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1">
                      <button
                        onClick={() =>
                          setLines((p) =>
                            p.map((x) =>
                              x.part.id === l.part.id
                                ? { ...x, qty: Math.max(0.01, x.qty - 1) }
                                : x,
                            ),
                          )
                        }
                        className="w-8 h-8 rounded-lg border hover:bg-muted"
                      >
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          setLines((p) =>
                            p.map((x) =>
                              x.part.id === l.part.id
                                ? { ...x, qty: Math.max(0.01, Number(e.target.value)) }
                                : x,
                            ),
                          )
                        }
                      ...
                      <button
                        onClick={() =>
                          setLines((p) =>
                            p.map((x) => (x.part.id === l.part.id ? { ...x, qty: x.qty + 1 } : x)),
                          )
                        }
                        className="w-8 h-8 rounded-lg border hover:bg-muted"
                      >
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        setLines((p) =>
                          p.map((x) =>
                            x.part.id === l.part.id
                              ? { ...x, unit_cost: Math.max(0, Number(e.target.value)) }
                              : x,
                          ),
                        )
                      }
                    ...
                    />
                    <div className="w-24 text-left font-semibold text-sm">
                      {formatSDG(l.qty * l.unit_cost)}
                    </div>
                    <button
                      onClick={() => setLines((p) => p.filter((x) => x.part.id !== l.part.id))}
        ...
        <aside className="bg-card border rounded-2xl p-4 space-y-3 h-fit lg:sticky lg:top-20">
          ...
          <div className="border-t pt-3 space-y-1 text-sm">
            <div className="flex justify-between text-muted-foreground">
              <span>الإجمالي</span>
              <span>{formatSDG(total)}</span>
            </div>
            <div className="flex justify-between text-muted-foreground">
              <span>المدفوع</span>
              <span>- {formatSDG(paid)}</span>
            </div>
            <div className="flex justify-between font-bold text-base pt-2 border-t">
              <span>المستحق</span>
              <span className={total - paid > 0 ? "text-destructive" : "text-success"}>
                {formatSDG(Math.max(0, total - paid))}
              </span>
            </div>
          </div>
          <Btn
