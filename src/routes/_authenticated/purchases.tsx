...
type Purchase = {
  id: string; invoice_no: number; total: number; paid: number; created_at: string;
...
function PurchasesPage() {
  ...
  const { data: list = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("purchases")
        .select("id,invoice_no,total,paid,created_at, suppliers(name)")
  ...
  return (
    <div className="p-4 lg:p-6 max-w-6xl mx-auto">
      <PageHeader title="فواتير المشتريات" subtitle={`${list.length} فاتورة`}
        actions={<Btn onClick={() => setCreating(true)}><Plus className="w-4 h-4 inline ml-1" />فاتورة شراء</Btn>} />
...
function NewPurchase({ onDone, onCancel }: { onDone: () => void; onCancel: () => void }) {
  ...
  const { data: parts = [] } = useQuery({
    queryKey: ["parts-lite-cost"],
    queryFn: async () => {
      const { data, error } = await supabase.from("parts").select("id,code,name,cost_price").order("name");
  ...
  const results = useMemo(() =>
    !q ? [] : parts.filter((p) => p.code.includes(q) || p.name.includes(q)).slice(0, 8), [q, parts]);

  const addPart = (p: Part) => {
    setLines((prev) => {
      const i = prev.findIndex((l) => l.part.id === p.id);
      if (i >= 0) { const nx = [...prev]; nx[i] = { ...nx[i], qty: nx[i].qty + 1 }; return nx; }
      return [...prev, { part: p, qty: 1, unit_cost: Number(p.cost_price) }];
    });
    setQ("");
  };
  ...
  return (
    <div className="p-4 lg:p-6 max-w-6xl mx-auto">
      <PageHeader title="فاتورة شراء جديدة"
        actions={<Btn variant="outline" onClick={onCancel}>إلغاء</Btn>} />
          ...
          <div className="relative">
            <div className="flex items-center gap-2 h-12 px-3 rounded-xl border bg-card">
              ...
              <input value={q} onChange={(e) => setQ(e.target.value)} autoFocus
                placeholder="ابحث عن قطعة لإضافتها..." className="flex-1 bg-transparent outline-none" />
            </div>
            {results.length > 0 && (
                  ...
                  <div key={l.part.id} className="p-3 flex items-center gap-3">
                    ...
                    <div className="flex items-center gap-1">
                      ...
                      <input type="number" step="0.01" value={l.qty}
                        onChange={(e) => setLines((p) => p.map((x) => x.part.id === l.part.id ? { ...x, qty: Math.max(0.01, Number(e.target.value)) } : x))}
                    ...
                    <input type="number" step="0.01" value={l.unit_cost}
                      onChange={(e) => setLines((p) => p.map((x) => x.part.id === l.part.id ? { ...x, unit_cost: Math.max(0, Number(e.target.value)) } : x))}
