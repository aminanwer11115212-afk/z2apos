...
function rangeFor(preset: Preset, from: string, to: string): { from: string; to: string } {
  ...
  if (preset === "week") {
    ...
    return { from: iso(w), to: iso(t) };
  }
  if (preset === "month") {
    const m = new Date(t.getFullYear(), t.getMonth(), 1);
    return { from: iso(m), to: iso(t) };
  }
  if (preset === "year") {
    const y = new Date(t.getFullYear(), 0, 1);
    return { from: iso(y), to: iso(t) };
  }
  return { from, to };
}
...
function Reports() {
  ...
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      let salesQ = supabase
        .from("sales")
        .select(
          "id,total,discount,tax_amount,paid,payment_method,created_at,created_by,customer_id",
        )
      ...
      const { data: purchases } = await supabase
        .from("purchases")
        .select("id,total,paid,payment_method,created_at")
        .gte("created_at", start)
        .lte("created_at", end);

      let partsQ = supabase
        .from("parts")
        .select("name,category,quantity,cost_price,sell_price,min_quantity");
      ...
      let items: {
        qty: number;
        subtotal: number;
        parts: { name: string; cost_price: number; category: string | null } | null;
      }[] = [];
      if (saleIds.length > 0) {
      ...
      for (const s of sales ?? []) {
        ...
        if (initial > 0)
          addToMethod(
            (s as { payment_method?: string | null }).payment_method ?? "cash",
            "in",
            initial,
          );
      }
      for (const pu of purchases ?? []) {
        ...
        if (initial > 0)
          addToMethod(
            (pu as { payment_method?: string | null }).payment_method ?? "cash",
            "out",
            initial,
          );
      }
      ...
      const stockValueCost = (parts ?? []).reduce(
        (s, p) => s + Number(p.quantity) * Number(p.cost_price),
        0,
      );
      const stockValueSell = (parts ?? []).reduce(
      ...
      return {
        ...
        stockValueCost,
        stockValueSell,
        stockCount: parts?.length ?? 0,
        lowStock,
        salesTax,
        salesGross: salesNet + salesTax,
        cogs,
        profit: salesNet - cogs,
        topProducts,
        methodMap,
      };
    },
  ...
  const exportCsv = () => {
    ...
    const rows: (string | number)[][] = [
      ...
      ["الدفعات حسب الطريقة"],
      ["الطريقة", "تحصيل", "سداد"],
      ...Object.entries(data.methodMap).map(
        ([m, v]) => [paymentMethodLabel(m), v.in, v.out] as (string | number)[],
      ),
    ];
      ...
      <div className="print-area space-y-6">
        <div className="hidden print:flex items-center justify-between border-b pb-3">
          ...
          <div className="text-left text-xs muted-print text-muted-foreground">
            ...
            <div>
              طُبعت:{" "}
              {new Date().toLocaleString("ar-SD", { dateStyle: "short", timeStyle: "short" })}
            </div>
          </div>
        </div>
        ...
        <Section title="المبيعات">
          ...
          {!!data?.salesTax && (
            <Stat label="الإجمالي شامل الضريبة" value={formatSDG(data.salesGross)} />
          )}
          <Stat label="المحصّل" value={formatSDG(data?.salesCollected ?? 0)} />
          <Stat
            label="المتبقي"
            value={formatSDG((data?.salesGross ?? 0) - (data?.salesCollected ?? 0))}
            highlight
          />
        </Section>
        ...
        <Section title="الربحية">
          ...
          <Stat
            label="هامش الربح"
            value={`${data && data.salesNet ? Math.round((data.profit / data.salesNet) * 100) : 0}%`}
          />
        </Section>
            ...
            <table className="w-full text-sm">
              <thead className="text-xs muted-print text-muted-foreground border-b">
                <tr>
                  ...
                  <th className="text-left py-2">الإيراد</th>
                </tr>
              </thead>
            ...
            <table className="w-full text-sm">
              <thead className="text-xs muted-print text-muted-foreground border-b">
                <tr>
                  ...
                  <th className="text-center py-2">تحصيل</th>
                  <th className="text-left py-2">سداد</th>
                </tr>
              </thead>
              <tbody className="divide-y">
                {Object.entries(data.methodMap).map(([m, v]) => (
                  <tr key={m}>
                    ...
                    <td className="py-2 text-center text-success font-semibold">
                      {formatSDG(v.in)}
                    </td>
                    <td className="py-2 text-left text-destructive font-semibold">
                      {formatSDG(v.out)}
                    </td>
                  </tr>
                ))}
        ...
        <Section title="المشتريات">
          ...
          <Stat
            label="المستحق"
            value={formatSDG((data?.purchasesTotal ?? 0) - (data?.purchasesPaid ?? 0))}
            highlight
          />
        </Section>
