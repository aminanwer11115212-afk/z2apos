  ...
  const { data: rows = [] } = useQuery({
    ...
    queryFn: async () => {
      const [salesRes, paymentsRes] = await Promise.all([
        supabase
          .from("sales")
          .select("id,invoice_no,total,discount,tax_amount,paid,created_at,notes")
          .eq("customer_id", id)
          .order("created_at"),
        supabase
          .from("payments")
          .select("id,amount,method,account_name,notes,created_at,sale_id")
          .eq("customer_id", id)
          .eq("direction", "in")
          .order("created_at"),
      ]);
      ...
      for (const p of paymentsRes.data ?? []) {
        list.push({
          ...
          id: p.id,
          date: p.created_at,
          ref: `دفعة${p.sale_id ? " (على فاتورة)" : ""}`,
          debit: 0,
          credit: Number(p.amount),
          note: `${paymentMethodLabel(p.method)}${p.account_name ? " · " + p.account_name : ""}${p.notes ? " · " + p.notes : ""}`,
        });
      }
  ...
  const waHref = cust.phone
    ? whatsappUrl(
        ...
        renderTemplate(settings.waReminderTemplate, {
          name: cust.name,
          balance: formatSDG(cust.balance),
          ...
          actions={
            <div className="flex gap-2 flex-wrap items-center">
              ...
              <Btn variant="outline" onClick={() => setPayOpen(true)}>
                ...
                {Number(cust.balance) > 0
                  ? `تحصيل (${formatSDG(Number(cust.balance))})`
                  : "تسجيل دفعة"}
              </Btn>
              {waHref && (
        ...
        <table className="w-full text-sm border rounded-lg overflow-hidden">
          ...
          <tbody>
            {rows.map((r) => {
              ...
              return (
                <tr key={r.id} className="border-t">
                  ...
                  <td className={`p-2 font-semibold ${running > 0 ? "text-destructive" : ""}`}>
                    {formatSDG(running)}
                  </td>
            ...
            <tr className="border-t bg-muted font-semibold">
              ...
              <td className={`p-2 ${totals.balance > 0 ? "text-destructive" : ""}`}>
