...
function UsersPage() {
  ...
  return (
    <div className="p-4 lg:p-6 max-w-4xl mx-auto">
      <PageHeader
        title="المستخدمون"
        subtitle={`${data.length} مستخدم`}
        actions={
          <Btn onClick={() => newDlg.show()}>
                ...
                {data.map((u) => (
                  <tr key={u.user_id} className="border-t">
                    ...
                    <td className="p-3 whitespace-nowrap text-left">
                      <button
                        onClick={() => {
                          setPwTarget({ userId: u.user_id, email: u.email });
                          setNewPw("");
                          pwDlg.show();
                        }}
                      ...
                      <button
                        onClick={() => {
                          if (confirm(`حذف الحساب ${u.email}؟`)) mDel.mutate(u.user_id);
                        }}
      ...
      <Modal
        ...
        footer={
          <>
            <Btn variant="outline" onClick={newDlg.hide}>
              إلغاء
            </Btn>
            <Btn
              onClick={() => mCreate.mutate()}
              disabled={mCreate.isPending || !form.email || !form.password}
            >
              إنشاء
            </Btn>
          </>
      ...
      <Modal
        open={pwDlg.open}
        onClose={pwDlg.hide}
        title={`تغيير كلمة المرور — ${pwTarget?.email ?? ""}`}
        footer={
          <>
            <Btn variant="outline" onClick={pwDlg.hide}>
              إلغاء
            </Btn>
            <Btn onClick={() => mReset.mutate()} disabled={mReset.isPending || newPw.length < 6}>
              حفظ
            </Btn>
          </>
        }
