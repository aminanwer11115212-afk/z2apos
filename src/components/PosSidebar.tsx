...
export function PosSidebar(props: PosSidebarProps) {
  const {
    ...
    paymentMethod, onPaymentMethod, bankAccountId, onBankAccountId,
    txRef, onTxRef, discount, onDiscount, paid, onPaid, notes, onNotes,
    total, settings, isSeller, accounts, lines, onPayFull, onHold, onClear, onSave, savePending,
    ...
    <aside className="bg-card border rounded-xl p-3 space-y-2 h-fit lg:sticky lg:top-16 text-sm">
      <Field label="العميل (F4)">
        <div className="flex gap-1">
          <select ref={customerRef} value={customerId} onChange={(e) => onCustomerId(e.target.value)}
      ...
      {paymentMethod === "bank" && (
        <>
          ...
          <Field label="رقم العملية">
            <input value={txRef} onChange={(e) => onTxRef(e.target.value)} placeholder="اختياري"
      ...
      <div className="grid grid-cols-2 gap-2">
        ...
        <Field label="مدفوع">
          <input type="number" step="0.01" value={paid} onChange={(e) => onPaid(Number(e.target.value) || 0)}
            className="w-full h-10 px-2 rounded-lg border bg-background text-sm" />
        </Field>
      </div>

      <button type="button" onClick={onPayFull} disabled={tax.grand <= 0}
      ...
      <div className="flex gap-2 pt-1">
        <Btn variant="outline" onClick={onClear} className="h-9 text-sm px-3">مسح</Btn>
        <Btn variant="outline" onClick={onHold} disabled={lines.length === 0} className="h-9 text-sm px-3" title="F8">
          <Pause className="w-3.5 h-3.5 inline ml-1" />تعليق
        </Btn>
        <Btn onClick={onSave} disabled={savePending || lines.length === 0} className="flex-1 h-9 text-sm">
          حفظ (F9)
        </Btn>
      </div>
    </aside>
