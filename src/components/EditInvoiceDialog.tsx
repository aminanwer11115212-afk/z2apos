...
export function EditInvoiceDialog({ open, onClose, kind, invoice }: Props) {
  ...
  const [method, setMethod] = useState<PaymentMethod>(
    (invoice.payment_method as PaymentMethod) ?? "cash",
  );
  const [accountName, setAccountName] = useState<string>(
    invoice.account_name ?? parsed.account ?? "",
  );
    ...
    mutationFn: async () => {
      ...
      if (kind === "sale") {
        const { error } = await supabase
          .from("sales")
          .update({ ...basePatch, discount, tax_amount: taxAmount })
          .eq("id", invoice.id);
    ...
    >
      <div className="space-y-3">
        <p className="text-xs text-muted-foreground">
          يمكن تعديل الخصم والدفع وطريقة الدفع والملاحظات فقط. الأصناف والكميات لا تُعدَّل حفاظاً
          على المخزون.
        </p>
        ...
        <div className="p-3 rounded-lg bg-muted/50 text-sm space-y-1">
          ...
          {taxAmount > 0 && (
            <div className="flex justify-between">
              ...
              <span className="font-medium">{grand.toFixed(2)}</span>
            </div>
          )}
          <div className="flex justify-between">
            <span className="text-muted-foreground">المتبقي</span>
            <span className={`font-bold ${due > 0 ? "text-destructive" : "text-success"}`}>
              {due.toFixed(2)}
            </span>
          </div>
        </div>
