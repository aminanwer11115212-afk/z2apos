    ...
    <Modal open={open} onClose={onClose} title={title}
      ...
      <div className="space-y-3">
        <div className="p-3 rounded-lg bg-muted/50 text-sm flex items-center justify-between">
          ...
          <span className={`font-bold ${Number(party.balance) > 0 ? "text-destructive" : Number(party.balance) < 0 ? "text-success" : ""}`}>
            {formatSDG(party.balance)}{Number(party.balance) < 0 ? " (فائض)" : ""}
          </span>
        </div>
        ...
        {amount > 0 && (
          <div className="text-xs text-muted-foreground px-1">
            الرصيد بعد الحفظ: <span className={`font-semibold ${projectedBalance > 0 ? "text-destructive" : projectedBalance < 0 ? "text-success" : "text-foreground"}`}>
        ...
        <Field label="طريقة الدفع">
          <div className={`grid gap-1.5`} style={{ gridTemplateColumns: `repeat(${Math.max(1, enabledMethods.length)}, minmax(0, 1fr))` }}>
