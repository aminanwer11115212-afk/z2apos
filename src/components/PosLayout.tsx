...
type PosLayoutProps = {
  ...
  custDialog: {
    ...
    setForm: (v: { name: string; phone: string }) => void;
        ...
        actions={
          <div className="flex items-center gap-2">
            ...
            >
              ...
              المعلّقة
              {props.held.length > 0 && (
                <span className="min-w-5 h-5 px-1 rounded-full bg-primary text-primary-foreground text-[10px] flex items-center justify-center font-bold">
                  {props.held.length}
                </span>
              )}
            </button>
      ...
      <div className="grid lg:grid-cols-[1fr,320px] gap-3">
        <div className="space-y-3">
          <div className="flex items-center gap-2 h-11 px-3 rounded-xl border bg-card">
            ...
            />
            {props.q && (
              <button onClick={() => props.setQ("")} className="text-xs text-muted-foreground">
                ✕
              </button>
            )}
          ...
          {props.lines.length === 0 && (
            <div className="text-center p-8 text-muted-foreground border rounded-xl bg-card">
              ...
              <Btn onClick={() => props.searchRef.current?.focus()}>
        ...
        <PosSidebar
          ...
          lines={props.lines}
          onPayFull={props.onPayFull}
          onHold={props.onHold}
          onClear={props.onClear}
          onSave={props.onSave}
          savePending={props.savePending}
          customerRef={props.customerRef}
          methodRef={props.methodRef}
          accountRef={props.accountRef}
        />
      </div>

      <PosCustomerDialog
        isOpen={props.custDialog.isOpen}
        onClose={props.custDialog.close}
        form={props.custDialog.form}
        setForm={props.custDialog.setForm}
        save={props.custDialog.save}
      />
      <PosHeldDialog
        open={props.holdOpen}
        onClose={() => props.setHoldOpen(false)}
        held={props.held}
