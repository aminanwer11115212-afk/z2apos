...
export function usePos() {
  ...
  const addPart = (p: PosPart) => {
    const available = Math.max(
      0,
      Number(p.quantity) - (lines.find((l) => l.part.id === p.id)?.qty ?? 0),
    );
    if (available < 1) {
      toast.error(`لا يوجد رصيد كافٍ من ${p.name}`);
      return;
    }
    setLines((prev) => {
  ...
  const setQty = (id: string, qty: number) => {
    const max = Number(parts.find((x) => x.id === id)?.quantity ?? Infinity);
    setLines((p) =>
      p.map((l) => (l.part.id === id ? { ...l, qty: Math.max(0.01, Math.min(qty, max)) } : l)),
    );
  };
  ...
  onKeyRef.current = (e: KeyboardEvent) => {
    ...
    if (e.key === "F2" || (e.key === "/" && !inField)) {
      ...
      e.preventDefault();
      if (!isDigital) selectPaymentMethod("bank");
      ...
      e.preventDefault();
      if (lines.length && !save.isPending) save.mutate();
    } else if (e.key === "F8") {
      e.preventDefault();
      held.hold();
      ...
      if (last) {
        e.preventDefault();
        setQty(last.part.id, last.qty + 1);
      }
    } else if (!inField && (e.key === "-" || e.key === "_")) {
      const last = lines[lines.length - 1];
      if (last) {
        e.preventDefault();
        setQty(last.part.id, Math.max(0.01, last.qty - 1));
      }
    }
  };
